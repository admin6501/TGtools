...
async def start_keep_alive(event):
    ...
    global keep_alive_active, bot_active
...
async def start_auto_reply(event):
    ...
    global auto_reply_active, bot_active
...
async def status(event):
    ...
    global keep_alive_active, auto_reply_active, auto_reply_count, last_auto_reply_time, bot_active
    status_message = (
        f"**Bot Status:**\n"
        f"Auto-reply: {'Active' if auto_reply_active else 'Inactive'}\n"
        f"Keepalive: {'Active' if keep_alive_active else 'Inactive'}\n"
        f"Bot Active: {'Active' if bot_active else 'Inactive'}\n"
