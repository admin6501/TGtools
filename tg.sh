...
async def _send_pack_to_saved_messages_via_sticker(first_doc, title: str, admin_id: int):
    # «خود پک» با ارسال یک استیکر (بدون لینک)
    try:
        caption = f"Sticker pack created for admin {admin_id}\n{title}"
        await client.send_file("me", first_doc, caption=caption)
    except Exception:
        pass
    ...
    for _ in range(12):
        ...
        try:
            ...
            packs[admin_key] = {
                "short_name": candidate,
                "title": title,
                "sent_pack_to_saved": False,
                "seen": (packs.get(admin_key) or {}).get("seen", []),
            }
            state["packs"] = packs
            _save_pack_state()

            # معرفی پک فقط یک‌بار در Saved Messages
            try:
                await _send_pack_to_saved_messages_via_sticker(first_doc, title, admin_id)
                packs[admin_key]["sent_pack_to_saved"] = True
                state["packs"] = packs
                _save_pack_state()
            except Exception:
                pass
...
# ---------- Fancy sticker renderer (text stays, style improved) ----------
...
def _add_noise(img: Image.Image, rng: random.Random, amount: int = 12):
    w, h = img.size
    ...
    for y in range(h):
        for x in range(w):
            ...
            a = rng.randint(18, 40)
            npx[x, y] = (v & 255, v & 255, v & 255, a)
    return Image.alpha_composite(img, noise)

def _draw_blobs(base: Image.Image, rng: random.Random, count: int = 7):
    w, h = base.size
    ...
    for _ in range(count):
        ...
        rx = rng.randint(90, 230)
        ry = rng.randint(90, 230)
        col = (
            rng.randint(90, 255),
            rng.randint(90, 255),
            rng.randint(90, 255),
...
def _glow(layer: Image.Image, radius: int = 10, alpha: int = 110):
    a = layer.split()[-1]
    blur = a.filter(ImageFilter.GaussianBlur(radius=radius))
    glow_img = Image.new("RGBA", layer.size, (255, 255, 255, alpha))
    glow_img.putalpha(blur)
    return glow_img

def _fit_font(draw: ImageDraw.ImageDraw, text: str, font_candidates: list, max_w: int, max_h: int, start: int = 58, min_size: int = 20):
    def wrap_lines(txt, width_chars: int):
        lines = []
    ...
    for fs in range(start, min_size - 1, -2):
        ...
        width_chars = max(14, min(30, max_w // max(10, fs)))
        lines = wrap_lines(text, width_chars)
        multiline = "\n".join(lines)
        w, h = bbox(multiline, fnt)
        if w <= max_w and h <= max_h:
            return fnt, multiline

    fnt = ImageFont.load_default()
    return fnt, "\n".join((textwrap.wrap(text, width=22) or [text])[:14])

def build_cool_text_sticker(text: str, size: int = 512) -> Image.Image:
    rng = _seeded_rng(text)

    palettes = [
        ((255, 0, 132), (0, 206, 255)),
        ((26, 188, 156), (52, 152, 219)),
        ((155, 89, 182), (46, 204, 113)),
    ...
    base = _gradient_bg(size, c1, c2, angle)
    base = _add_noise(base, rng, amount=12)
    base = _draw_blobs(base, rng, count=rng.randint(6, 9))

    gloss = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    gd = ImageDraw.Draw(gloss)
    gd.ellipse((-120, -180, size + 260, size // 2), fill=(255, 255, 255, 38))
    ...
    x1 = x0 + box_w
    y1 = y0 + box_h
    bd.rounded_rectangle((x0, y0, x1, y1), radius=44, fill=(10, 12, 16, 120), outline=(255, 255, 255, 80), width=2)
    box = box.filter(ImageFilter.GaussianBlur(radius=0.6))
    base = Image.alpha_composite(base, box)
    ...
    max_w = box_w - 2 * 26
    max_h = box_h - 2 * 26
    font, multiline = _fit_font(d, text.strip(), font_candidates, max_w, max_h, start=62, min_size=20)

    if hasattr(d, "multiline_textbbox"):
        b = d.multiline_textbbox((0, 0), multiline, font=font, spacing=10, align="center")
        tw, th = (b[2] - b[0]), (b[3] - b[1])
    else:
        tw, th = d.multiline_textsize(multiline, font=font, spacing=10)

    tx = (size - tw) // 2
    ty = (size - th) // 2

    d.multiline_text((tx + 4, ty + 6), multiline, font=font, fill=(0, 0, 0, 160), spacing=10, align="center")
    d.multiline_text(
        (tx, ty),
        multiline,
        font=font,
    ...
    base = Image.alpha_composite(base, _glow(overlay, radius=12, alpha=90))
    base = Image.alpha_composite(base, overlay)

    # rounded sticker mask
    mask = Image.new("L", (size, size), 0)
    md = ImageDraw.Draw(mask)
    md.rounded_rectangle((22, 22, size - 22, size - 22), radius=120, fill=255)
    shaped = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    shaped.paste(base, (0, 0), mask)

    # outline + shadow
    alpha = shaped.split()[-1]
    outline = alpha.filter(ImageFilter.MaxFilter(size=19))
    outline_img = Image.new("RGBA", (size, size), (255, 255, 255, 255))
    outline_img.putalpha(outline)

    shadow = outline.filter(ImageFilter.GaussianBlur(radius=9))
    shadow_img = Image.new("RGBA", (size, size), (0, 0, 0, 120))
    shadow_img.putalpha(shadow)

    canvas = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    canvas = Image.alpha_composite(canvas, ImageChops.offset(shadow_img, 6, 10))
...
async def text_to_sticker(event):
    ...
    # ساخت استیکر زیباتر با متن
    img = build_cool_text_sticker(text_to_render, size=512)
