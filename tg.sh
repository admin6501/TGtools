...
install_system_packages() {
  ...
  if is_termux; then
    ...
    # DejaVu font package name can differ across termux builds; try a couple of common ones
  ...
  if have apt-get; then
    ...
    # Optional deps for better sticker rendering / conversion fallback
...
ensure_python_and_pip() {
  ...
  # Prefer python -m pip (more reliable than pip3 on some systems)
...
pip_install() {
  # Try global; if permission fails, fallback to --user
  local pkgs=("$@")
  if python3 -m pip install --upgrade pip >/dev/null 2>&1; then
    :
  else
    warn "Could not upgrade pip globally; continuing."
  fi
  ...
  python3 -m pip install --user "${pkgs[@]}" || return 1
}
...
# دریافت اطلاعات مورد نیاز از کاربر (بعد از نصب پیش‌نیازها)
...
# ساخت فایل پایتون (بدون تزریق مستقیم ورودی‌ها داخل کد)
cat > "$PYTHON_FILE" <<'PY'
...
from telethon.errors import StickersetNameOccupiedError
...
async def _get_or_create_pack_for_admin(admin_id: int, first_doc, emoji: str = "📝") -> str:
    ...
    for _ in range(10):
        ...
        try:
            ...
            _save_pack_state()
            return candidate
        except StickersetNameOccupiedError:
            continue
        except Exception:
            raise
...
async def text_to_sticker(event):
    ...
    # Font search (includes common Termux path)
    font_size = 52
    font = None
    font_candidates = [
        "DejaVuSans.ttf",
    ...
    for fs in range(font_size, 18, -2):
        try:
            fnt = ImageFont.truetype(font_candidates[0], fs)
        except Exception:
            try:
                fnt = ImageFont.truetype(font_candidates[1], fs)
            except Exception:
