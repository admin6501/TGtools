...
API_KEY = 'AIzaSyA6TUL_nHLoF5FtafHYLTYB8Fu2pnPZCa4'
CSE_ID = '43cd89242865f4298'
...
async def google_search_handler(event):
    query = event.pattern_match.group(1)
    results = google_search(query)
    if results:
        reply_message = "\n".join([f"{item['title']} - {item['link']}" for item in results[:5]])  # نمایش ۵ نتیجه اول
